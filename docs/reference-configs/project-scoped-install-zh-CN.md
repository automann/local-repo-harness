...
- 能运行 `bash`。
- 已安装 Bun。
- 目标项目根目录已经有 `package.json`。如果没有，先建立项目级 package 边界；不要在没有 `package.json` 的子目录里直接运行 `bun add`，否则 Bun 会沿用上级 package 边界。
...
test -f package.json || bun init -y
```
...
先确认当前目录就是目标项目根目录，并且这里已经有自己的 `package.json`。这一步必须早于安装 CodeGraph；如果当前目录没有 `package.json`，`bun add` 可能沿用上级 package 边界，把 CodeGraph 装到上级目录。
...
先确认当前目录就是目标项目根目录，并且这里已经有自己的 `package.json`：
