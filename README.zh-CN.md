...
contract 开始，并在 dry-run 看起来正确之前保持 host runtime 写入关闭。
...
也不会安装 external skills。已有仓库使用 `local-repo-harness adopt`；新项目或新模块使用
...
`.ai/harness/runtime/`。Codex 仍需要你在 Codex Settings 中信任项目 hooks file。
...
- 调试顺序：user-level adapter config -> `local-repo-harness-hook` 或 fallback `local-repo-harness hook` -> route registry -> `.ai/hooks/*`。
...
- npm package：`local-repo-harness@0.5.2`
- Generated workflow stamp：`local-repo-harness@0.5.2+template@0.5.2`
