...
describe("Migration script contract", () => {
  ...
  test("should migrate workflow files and runtime ignore block", () => {
    ...
    expect(sharedLib).toContain("REPO_HARNESS_VCS_PROFILE");
    expect(sharedLib).toContain("REPO_HARNESS_TRACKED_WHITELIST");
    expect(sharedLib).toContain("policy.vcs");
    expect(sharedLib).toContain("tracked_whitelist");
    expect(sharedLib).toContain("local_only_manifest");
    expect(script).toContain("VCS profile:");
    ...
    expect(workflowContract).toContain("defaultVcsProfile");
    expect(workflowContract).toContain("project-local-install");
    expect(workflowContract).toContain("rootGitignorePrecedence");
  ...
  test("project CodeGraph MCP policy emits project-local commands without user PATH expansion", () => {
    const repo = mkdtempSync(join(tmpdir(), "migration-policy-codegraph-project-"));
    try {
      writeFileSync(join(repo, "package.json"), JSON.stringify({ name: "demo", scripts: {} }, null, 2));
