...
describe("workflow contract manifest", () => {
  ...
  test("helper inventory should come from the workflow contract", () => {
    ...
    expect(contract.compatibility.repoLocalFirst).toBe(true);
    expect(contract.compatibility.repoWorkflowStateLocalFirst).toBe(true);
    expect(contract.compatibility.defaultHelperSource).toBe("package");
    expect(contract.compatibility.defaultHookSource).toBe("package-or-project-runtime");
    expect(contract.externalTooling?.repoHarnessSkills?.supportedScopes).toEqual(["project", "user", "none"]);
    expect(contract.externalTooling?.repoHarnessSkills?.projectPaths.codex).toBe(".agents/skills");
    expect(contract.externalTooling?.repoHarnessSkills?.projectPaths.claude).toBe(".claude/skills");
    expect(contract.externalTooling?.repoHarnessSkills?.crossReview.codexHostSkill).toBe("claude-review");
    expect(contract.externalTooling?.skillsCli?.projectInstall).toBe("supported");
    expect(contract.externalTooling?.skillsCli?.globalFlag).toBe("-g");
    ...
    expect(contract.externalTooling?.waza?.projectInstall).toBe("skills-cli-without-global-flag");
    ...
    expect(contract.externalTooling?.diagramDesign?.projectInstall).toBe("skills-cli-without-global-flag");
    expect(contract.externalTooling?.codegraph?.indexScope).toBe("project");
    expect(contract.externalTooling?.codegraph?.mcpScopes).toEqual(["project", "user", "none"]);
    expect(contract.externalTooling?.codegraph?.projectMcp.codexConfigPath).toBe(".codex/config.toml");
    expect(contract.externalTooling?.codegraph?.projectMcp.claudeConfigPath).toBe(".mcp.json");
    expect(contract.externalTooling?.gbrain?.projectOnlyMode).toBe("manifest-only");
    expect(contract.externalTooling?.gbrain?.mcp).toBe("candidate-disabled");
    ...
    expect(contract.hostAdapters?.runtimeModes).toContain("global-path");
