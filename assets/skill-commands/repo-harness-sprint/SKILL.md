...
4. Route `run` (incremental, one backlog task per invocation):
   ...
   - Execute the slice as usual (implement, `/check`, external acceptance, `bash scripts/contract-worktree.sh finish`); finish back-fills the backlog row warn-only. Before closeout, convert discovered edge cases into task-local `commands_succeed` or `commands_fail` contract gates. Do not put workflow/meta gates such as `verify-sprint`, `check-task-workflow --strict`, or `contract-worktree finish` inside `commands_succeed` or `commands_fail`; run those outer gates separately after the review is terminal pass. `manual_checks` supports only the documented verifier enum; put custom manual claims in review notes. Then require review `Status: Reviewed`, `Recommendation: pass`, and passing or constrained manual-override external acceptance. If strict workflow reports stale handoff/resume state, run `local-repo-harness run prepare-handoff workflow-sync`, then rerun `check-task-workflow --strict`.
