...
pi_default_runtime_block() {
  ...
  if [[ "${REPO_HARNESS_VCS_SCOPE:-tracked}" == "local" || "${REPO_HARNESS_INSTALL_STATE_VCS_SCOPE:-tracked}" == "local" ]]; then
    runtime_entries="$(printf '%s\n' "$runtime_entries" | grep -Fxv '!.codex/hooks.json' || true)"
  fi
